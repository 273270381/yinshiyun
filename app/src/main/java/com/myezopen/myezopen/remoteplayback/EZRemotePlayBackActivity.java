 ...
 * @warning 这里写本文件的相关警告
 */
package com.myezopen.myezopen.remoteplayback;
...
import com.myezopen.myezopen.Utils.RemoteListUtil;
...
//                handleSearchEZDeviceFileSuccess();
    ...
    private void handleSearchEZCloudFileSuccess(Calendar seletedTime) {
        if (mEZMediaPlayer == null) {
            mEZMediaPlayer = MyApplication.getOpenSDK().createPlayer(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo());
            if (mEZMediaPlayer == null)
                return;
...
//        mRemotePlayBackCaptureRl.setVisibility(View.VISIBLE);
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    mCloudFileList = MyApplication.getOpenSDK().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(),starttime,endtime);
    ...
    private void searchEZCloudFileList(final Calendar seletedTime) {
        new Thread() {
            ...
            public void run() {
                try {
                    mEZCloudFileList = MyApplication.getOpenSDK().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(), mStartTime, mEndTime);
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    mEZDeviceFileList = MyApplication.getOpenSDK().searchRecordFileFromDevice(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(), startTime, endTime);
                    if (mEZDeviceFileList != null && mEZDeviceFileList.size() > 0) {
                    ...
                    mEZCloudFileList = MyApplication.getOpenSDK().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(), startTime, endTime);
