package com.myezopen.myezopen.devicemgt;
...
//            tv.setGravity(Gravity.CENTER);
    ...
    private class GetDeviceInfoTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
        	try {
        		mDeviceVersion = MyApplication.getOpenSDK().getDeviceVersion(mEZDeviceInfo.getDeviceSerial());
        		return true;
        	} catch (BaseException e) {;
                ErrorInfo errorInfo = (ErrorInfo) e.getObject();
...
//            }
    ...
    private class SetDefenceTask extends AsyncTask<Boolean, Void, Boolean> {
		...
		protected Boolean doInBackground(Boolean... params) {
			...
			try {
				result = MyApplication.getOpenSDK().setDefence(mEZDeviceInfo.getDeviceSerial(), bSetDefence? EZConstants.EZDefenceStatus.EZDefence_IPC_OPEN:
    ...
    private class OpenEncryptTask extends AsyncTask<Boolean, Void, Boolean> {
        ...
        protected Boolean doInBackground(Boolean... params) {
            ...
            try {
            	MyApplication.getOpenSDK().setDeviceEncryptStatus(mEZDeviceInfo.getDeviceSerial(),mValidateCode,isEnableEncrypt);
...
//            	EZCameraInfo cameraInfo = params[0];
                MyApplication.getOpenSDK().deleteDevice(mEZDeviceInfo.getDeviceSerial());
                return true;
            } catch (BaseException e) {
                ErrorInfo errorInfo = (ErrorInfo) e.getObject();
                mErrorCode = errorInfo.errorCode;
